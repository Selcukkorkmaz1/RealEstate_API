...
namespace RealEstate_UI.Areas.EstateAgent.Controllers
{
    ...
    public class MyAdvertsController : Controller
    {
        ...
        public async Task<IActionResult> ActiveAdverts()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:44327/api/Product/ProductAdvertsListByEmployeeByTrue?id=" + id);
        ...
        public async Task<IActionResult> PassiveAdverts()
        {
            var id = _loginService.GetUserId;
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:44327/api/Product/ProductAdvertListByEmployeeByFalse?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<List<ResultProductAdvertListWithCategoryByEmployeeDto>>(jsonData);
        ...
        public async Task<IActionResult> CreateAdvert(CreateProductDto createProductDto)
        {
            ...
            var id = _loginService.GetUserId;
            createProductDto.EmployeID = int.Parse(id);

            var client = _httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(createProductDto);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PostAsync("https://localhost:44327/api/Product", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
